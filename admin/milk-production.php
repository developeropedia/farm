...
$milk_production = findAllByQuery("SELECT *, mp.id AS mpId, s.id AS sId, s.name AS sName, cn.name AS cnName FROM milk_production mp INNER JOIN cattle c on mp.cattle_id = c.id INNER JOIN stations s ON s.id = c.station_id INNER JOIN countries cn ON cn.id = s.country_id ORDER BY mp.created_at DESC");
                                ...
                                <?php if(!empty($milk_production)): ?>
                                    <?php $count = 1; foreach ($milk_production as $mp): ?>
                                        <tr>
                                            ...
                                            <td class="product-table-text"><?php echo $mp->cnName ?></td>
